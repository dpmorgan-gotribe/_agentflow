...
- **Self-review hooks** for output quality validation (integrated in Step 12a)
- **CLI-first AI execution** via AIProvider abstraction (CLI for development, API for production)
...
import { getAIProvider, AIProvider, AIProviderResponse } from '../ai';
...
export abstract class BaseAgent {
    ...
    try {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          // Track token usage (available in API mode, may be undefined in CLI mode)
   ...
   */
  protected async spawnSubagent(role: string, task: string): Promise<string> {
