...
- **Self-validate output** against task requirements (see Step 12a)
...
## 5. Design Specification Propagation

> **Implementation Note (2025-12-30):** This section documents how approved designs are propagated to subsequent generation steps to ensure consistency.

### Problem

When a design is selected (e.g., "Minimalist Design" with purple/coral/teal colors), subsequent kitchen sink and screen mockups may not match. Without explicit design specification injection, the AI generates inconsistent colors, fonts, and component styles.

### Solution: `buildDesignSpec()` Function

The `buildDesignSpec()` function in `src/core/design-workflow.ts` generates a comprehensive design specification that is injected into every AI prompt that generates UI:
...
:root {
  --color-primary: ${tokens?.colors.primary || palette[0] || '#6B4C9A'};
  --color-secondary: ${tokens?.colors.secondary || palette[1] || '#E8736C'};
  --color-accent: ${tokens?.colors.accent || palette[2] || '#3AAFA9'};
  --color-background: ${tokens?.colors.background || '#FDF9F3'};
  --color-surface: ${tokens?.colors.surface || '#FFFFFF'};
  --color-text: ${tokens?.colors.text || '#2D3436'};
  /* ... typography, spacing, border-radius, shadows ... */
}

## REQUIRED CLASS USAGE
- Buttons: .btn, .btn-primary (use --color-primary), .btn-secondary
- Cards: .card (use --color-surface, --shadow-md, --radius-md)
- Headers: h1-h6 (use --font-heading, --color-text)
`;
}
```
...
Approved Design Option
        ↓
extractDesignTokensFromHtml() → DesignTokens
        ↓
buildDesignSpec(design) → String specification
        ↓
Injected into prompts for:
├── Kitchen Sink generation (createKitchenSink)
└── Screen Mockup generation (generateScreenMockup) × N screens
```
...
|----------|---------|----------|
| `buildDesignSpec()` | Generate CSS variables + design instructions | Lines 378-440 |
| `extractComponentSnippets()` | Extract button/card/header patterns | Lines 442-490 |
| `listKitchenSinkClasses()` | List available CSS classes from kitchen sink | Lines 492-534 |

### Prompt Integration

**Kitchen Sink Prompt:**
```typescript
const systemPrompt = `You are creating a Kitchen Sink / Style Guide.

${buildDesignSpec(design)}

## CRITICAL REQUIREMENTS
1. START your CSS with the EXACT :root variables above
2. Use var(--color-primary), var(--color-secondary), etc. throughout
3. Match the ${design.mood} mood in spacing and visual weight
`;
```

**Screen Mockup Prompt:**
```typescript
const systemPrompt = `You are creating the "${screen.name}" screen.

${buildDesignSpec(design)}

## USE KITCHEN SINK CLASSES
${listKitchenSinkClasses(kitchenSink)}

CRITICAL: Use the EXACT colors from the design spec.
`;
```
...
- Kitchen sink uses EXACT colors from approved design
- All screens use consistent CSS variables
- Component styles (buttons, cards) match across all screens
- Design mood is reflected consistently
