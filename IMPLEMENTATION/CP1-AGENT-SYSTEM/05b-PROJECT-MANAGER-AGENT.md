...
- **Self-validate work breakdown** against original requirements (see Step 12a)

---

## Response Format Flexibility

> **Implementation Note (2025-12-30):** The PM agent may return responses with varying field names depending on the prompt and context. The parsing code must handle these variations robustly.

### Field Name Variations

| Expected Field | Alternate Names to Check | Fallback Strategy |
|---------------|--------------------------|-------------------|
| `id` | - | Generate from `name`: `name.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '')` |
| `category` | `type` | Use `s.category || s.type || 'public'` |
| `name` | `title`, `label` | Use first available |
| `description` | `desc`, `summary` | Use first available or default to name |

### Parsing Strategies

The actual implementation (`src/core/design-workflow.ts`) uses multiple parsing strategies:

1. **JSON Code Block (Primary)** - Most reliable
   ```typescript
   const jsonMatch = response.content.match(/```json\s*([\s\S]*?)```/);
   ```

2. **Pipe-Delimited Format (Fallback)** - For older response patterns
   ```typescript
   const screensMatch = response.content.match(/---SCREENS---\s*([\s\S]*?)(?:---END_SCREENS---|---FLOW---|$)/);
   ```

3. **Markdown Heading Extraction (Last Resort)** - Extracts screens from markdown structure
