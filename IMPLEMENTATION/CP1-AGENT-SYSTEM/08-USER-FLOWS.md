...
## Implementation Note (2025-12-30)

> **Important:** The planned `src/flows/` directory structure was not created. Instead, user flow and screen identification logic is implemented in `src/core/design-workflow.ts` as part of the design-first workflow.

### Actual Implementation Location

| Planned File | Actual Implementation |
|--------------|----------------------|
| `src/flows/flow-generator.ts` | `src/core/design-workflow.ts` → `createUserFlows()` |
| `src/flows/flow-schema.ts` | Inline interfaces in `design-workflow.ts` |
| `src/flows/approval-gate.ts` | CLI flags `--no-approval`, `--select N` |
| `src/flows/diagram-renderer.ts` | Not implemented (flows output as JSON) |

### Screen Identification & Parsing

The PM agent identifies screens needed for user flows. Response parsing must handle variations:

**ID Generation Fallback:**
```typescript
// Generate id from name if PM doesn't provide one
...
**Category Fallback:**
```typescript
// PM sometimes returns 'type' instead of 'category'
const category = String(s.category || s.type || 'public');
```

**Parsing Strategies:**
1. JSON code block (primary) - ````json ... ```
2. Pipe-delimited format (fallback) - `---SCREENS--- id|name|desc|category`
3. Markdown heading extraction (last resort)

### Fallback Behavior

If no screens are parsed (JSON parsing fails or 0 valid screens), the system falls back to generic screens:
- Landing Page
- Sign Up
- Login
- Dashboard

This should be avoided by robust parsing - see the PM parsing fix in CP-RUNTIME-CHECKPOINT.md.

---

## Original Planned Deliverables (Reference)

The following was the original plan. See above for actual implementation:
...
## Original Planned File Structure (Reference)

```
src/flows/                        # NOT CREATED
├── flow-schema.ts               # → Now inline in design-workflow.ts
├── flow-generator.ts            # → Now createUserFlows() in design-workflow.ts
├── approval-gate.ts             # → Now CLI flags
├── diagram-renderer.ts          # → Not implemented
└── index.ts                     # → N/A
```
