...
## 6. Token Propagation to AI Prompts

> **Implementation Note (2025-12-30):** Design tokens extracted from approved designs must be propagated to subsequent AI calls to ensure consistent styling across all generated screens.

### Problem

Design tokens extracted from approved designs were not being passed to subsequent AI calls, causing inconsistent styling in generated screens. Each AI call would generate its own interpretation of colors and styles.

### Solution: Token Injection via `buildDesignSpec()`

The `buildDesignSpec()` function in `src/core/design-workflow.ts` creates a comprehensive specification that includes all tokens:
...
Approved Design HTML
        ↓
extractDesignTokensFromHtml(html, colorPalette) → DesignTokens
        ↓
{
  colors: { primary, secondary, accent, background, surface, text, ... },
  typography: { headingFont, bodyFont, monoFont, fontSizes, ... },
  spacing: { xs, sm, md, lg, xl, ... },
  borderRadius: { sm, md, lg, full },
  shadows: { sm, md, lg, xl }
}
        ↓
buildDesignSpec(design) → Markdown specification
        ↓
Injected into:
├── createKitchenSink() prompt
└── generateScreenMockup() prompt × N screens
```

### Token Extraction Function

```typescript
function extractDesignTokensFromHtml(
  html: string,
  colorPalette: string[]
): DesignTokens {
  // Extract colors from CSS custom properties
  const colorMatches = html.match(/--color-[\w-]+:\s*([^;]+)/g);

  // Extract fonts from font-family declarations
  const fontMatches = html.match(/font-family:\s*([^;]+)/g);

  // Map to DesignTokens structure
  return {
    colors: {
      primary: extractColor('primary', colorMatches) || colorPalette[0],
      secondary: extractColor('secondary', colorMatches) || colorPalette[1],
      accent: extractColor('accent', colorMatches) || colorPalette[2],
      // ... etc
    },
    typography: { ... },
    spacing: { ... },
...
### CSS Variables Block in Prompts

Every UI generation prompt includes a mandatory CSS variables block:
...
|------|----------|---------|
| `src/core/design-workflow.ts` | `extractDesignTokensFromHtml()` | Extract tokens from HTML |
| `src/core/design-workflow.ts` | `buildDesignSpec()` | Generate prompt specification |
| `src/core/design-workflow.ts` | `createKitchenSink()` | Uses tokens for style guide |
| `src/core/design-workflow.ts` | `generateScreenMockup()` | Uses tokens for each screen |
